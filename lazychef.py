...
def get_rating(user, recipe, lock, ratings, recommendations):
	rows = ratings.find({'user' : user, 'recipe' : recipe})
	if(rows.count() > 0):
		return rows[0]['rating']

	lock.acquire()
	rows = recommendations.find({'user' : user, 'recipe' : recipe})
	rating = 3
	if(rows.count() > 0):
	...
	def GET(self, user, ingredients):
		...
		for candidate in recommandation.keys():
			...
			for i in range(cursor.count()):
				#each req_ing is a row in db that this candidate recipe is related
				req_ing = cursor[i]
				candidate_rating = get_rating(user, req_ing['recipe_name'], web.ctx.LOCK, web.ctx.RATINGS, web.ctx.RECOMMENDATIONS)
				candidate_url = req_ing["url"]
				total_weight += req_ing["weight"]
				if req_ing["ingredient"] in recipe_has_ing[candidate]:
					recommandation[candidate] += req_ing["weight"]
